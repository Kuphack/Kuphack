...
public enum SupportedServer {
	...
	FLAGCLASH(client -> {
		if (!Kuphack.isOnMinehut())
			return false;
		return Kuphack.getScoreboard().stream().map(Text::getString).anyMatch(s -> s.contains("flagclash"));
	}), OVERCOOKED;
	...
	public static SupportedServer current() {
		return Kuphack.getServer();
	}
