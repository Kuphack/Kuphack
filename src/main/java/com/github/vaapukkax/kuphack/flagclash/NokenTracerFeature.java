...
public class NokenTracerFeature extends Feature implements EventHolder, WorldRenderEvents.AfterEntities {

	private static final Block NOKEN = Blocks.AMETHYST_CLUSTER;
	private final HashMap<BlockPos, Long> tracers = new HashMap<>();
	
	public NokenTracerFeature() {
	...
	public void afterEntities(WorldRenderContext context) {
		...
		this.tracers.forEach((pos, time) -> {
			...
			if (pos.isWithinDistance(context.camera().getPos(), 600))
				drawLine(context.matrixStack(), pos, context.camera(), new Color(255, 64, 255));
		});
		for (BlockPos pos : queue) {
			if (context.world().getBlockState(pos).getBlock() != NOKEN)
	...
	public void onEvent(ClientBlockBreakEvent e) {
		if (e.getBlock() == NOKEN) tracers.remove(e.getPos());
	}
	
	@EventMention
	public void onEvent(BlockUpdateEvent e) {
		if (e.getBlock() == NOKEN) tracers.put(e.getPos(), System.currentTimeMillis());
